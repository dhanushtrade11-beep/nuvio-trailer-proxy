...
async def get_custom_meta(content_type: str, content_id: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # Check if TMDB returned an API key error
            ...
            # 2. Safely parse the response using .get() to prevent KeyErrors
            if isinstance(tmdb_data, dict):
                if content_type == "movie":
                    movie_results = tmdb_data.get('movie_results', [])
                    if movie_results and len(movie_results) > 0:
                        name = movie_results[0].get('title', '')
                        year = movie_results[0].get('release_date', '')[:4]
                elif content_type == "series":
                    tv_results = tmdb_data.get('tv_results', [])
                    if tv_results and len(tv_results) > 0:
                        name = tv_results[0].get('name', '')
                        year = tv_results[0].get('first_air_date', '')[:4]

            best_video_id = None
            
            # 3. If a valid title was found, search YouTube
            if name:
            ...
            print(f"ERROR parsing TMDB/YouTube: {str(e)}")
